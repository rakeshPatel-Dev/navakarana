...
export default function AdminStreamsPage() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "live":
        return <Badge className="bg-red-100 text-red-700 border-red-250 font-bold rounded-md px-2 py-0.5">LIVE</Badge>;
      case "scheduled":
        return <Badge className="bg-blue-100 text-blue-700 border-blue-200 font-bold rounded-md px-2 py-0.5">SCHEDULED</Badge>;
      case "ended":
        return <Badge className="bg-stone-100 text-stone-600 border-stone-200 font-bold rounded-md px-2 py-0.5">ENDED</Badge>;
      case "cancelled":
        return <Badge className="bg-amber-100 text-amber-700 border-amber-250 font-bold rounded-md px-2 py-0.5">CANCELLED</Badge>;
      default:
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone-200/60 pb-5">
        <div>
          ...
          <p className="text-stone-500 text-xs mt-1">Monitor active transmissions, verify class details, and delete stream archives globally.</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-stone-100">
              {filteredStreams.map((stream) => (
                <tr key={stream.uuid} className="hover:bg-stone-50/50 transition-colors">
                  <td className="p-4 font-bold text-stone-850 max-w-xs truncate">{stream.title}</td>
                  <td className="p-4 font-semibold text-stone-750">{stream.teacherName}</td>
                  <td className="p-4 font-semibold text-stone-500 uppercase text-[10px]">{stream.category}</td>
            ...
            <div className="space-y-4 pt-2 text-xs">
              <div className="bg-stone-50 border border-stone-100 p-4 rounded-2xl space-y-3 font-medium">
                <div className="flex justify-between">
                  <span className="text-stone-400">Stream UUID:</span>
                  <span className="text-stone-750 font-mono">{selectedStream.uuid}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-stone-400">Title:</span>
                  <span className="text-stone-850 font-bold leading-normal">{selectedStream.title}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-stone-400">Instructor:</span>
                  <span className="text-stone-850 font-bold">{selectedStream.teacherName}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-stone-400">Category:</span>
                  <span className="text-stone-600 font-bold uppercase text-[10px]">{selectedStream.category}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span className="text-stone-400">Stream Cost:</span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-stone-600">{new Date(selectedStream.createdAt).toLocaleString()}</span>
                </div>
              </div>
              ...
              <DialogFooter className="pt-2">
                <Button
                  ...
                  variant="ghost"
                  className="w-full h-10 hover:bg-stone-50 rounded-xl font-semibold text-stone-500 text-xs"
                >
                  Close Inspector
                </Button>
              </DialogFooter>
